# Fix Camel Integration Capability Issue

Fix an issue from the Camel Integration Capability GitHub repository.

## Usage

```
/camel-integration-capability-fix-issue <issue>
```

**Arguments:**
- `<issue>` - GitHub issue number (e.g., `42`) or full URL (e.g., `https://github.com/wanaku-ai/camel-integration-capability/issues/42`)
...
- If full URL: extract number from path (e.g., `42` from `https://github.com/wanaku-ai/camel-integration-capability/issues/42`)
...
```bash
gh issue view <ISSUE_NUMBER> --repo wanaku-ai/camel-integration-capability --json number,title,body,state,labels,assignees,milestone
```

Or via API:

```bash
curl -s "https://api.github.com/repos/wanaku-ai/camel-integration-capability/issues/<ISSUE_NUMBER>" | jq '{
  number: .number,
  title: .title,
  body: .body,
  state: .state,
