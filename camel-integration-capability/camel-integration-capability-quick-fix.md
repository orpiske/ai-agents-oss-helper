# Quick Fix for Camel Integration Capability

Apply a quick fix to the Camel Integration Capability codebase without requiring a tracked issue. Intended for small changes such as CI fixes, documentation fixes, dependency upgrades, and other minor improvements.

## Usage

```
/camel-integration-capability-quick-fix <description>
```
...
**Examples:**
```
/camel-integration-capability-quick-fix upgrade Camel dependency version
/camel-integration-capability-quick-fix fix broken link in documentation
/camel-integration-capability-quick-fix update GitHub Actions workflow
```
...
If the change is too large or complex (e.g., new features, architectural changes, refactoring), stop and suggest creating an issue with `/camel-integration-capability-create-issue` instead.
...
1. **Branch**: Create from main with a descriptive name
   ...
   ```
   Use a short slug derived from the description (e.g., `quick-fix/upgrade-camel-dep`, `quick-fix/fix-docs-link`).
