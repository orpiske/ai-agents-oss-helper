...
Help users find an issue to contribute to Wanaku Capabilities Java SDK.

## Usage

```
/wanaku-capabilities-java-sdk-find-task
```
...
**Questions to ask:**
- Is this your first time contributing to Wanaku Capabilities Java SDK?
...
```bash
gh issue list --repo wanaku-ai/wanaku-capabilities-java-sdk --label "good first issue" --state open --limit 10 --json number,title,labels
```

Or via API:

```bash
curl -s "https://api.github.com/repos/wanaku-ai/wanaku-capabilities-java-sdk/issues?labels=good%20first%20issue&state=open&per_page=10" | jq '.[] | {number: .number, title: .title, labels: [.labels[].name]}'
```

#### For Experienced: Help Wanted

```bash
gh issue list --repo wanaku-ai/wanaku-capabilities-java-sdk --label "help wanted" --state open --limit 10 --json number,title,labels
```

Or via API:

```bash
curl -s "https://api.github.com/repos/wanaku-ai/wanaku-capabilities-java-sdk/issues?labels=help%20wanted&state=open&per_page=10" | jq '.[] | {number: .number, title: .title, labels: [.labels[].name]}'
```
...
Example:
```
1. #42 - Add retry logic to capability provider (Labels: help wanted, enhancement)
...
/wanaku-capabilities-java-sdk-fix-issue <ISSUE_NUMBER>

For example:
/wanaku-capabilities-java-sdk-fix-issue 42
```
...
- Direct users to `/wanaku-capabilities-java-sdk-fix-issue` for implementation
