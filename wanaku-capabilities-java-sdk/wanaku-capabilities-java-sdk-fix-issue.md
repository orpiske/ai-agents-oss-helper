# Fix Wanaku Capabilities Java SDK Issue

Fix an issue from the Wanaku Capabilities Java SDK GitHub repository.

## Usage

```
/wanaku-capabilities-java-sdk-fix-issue <issue>
```

**Arguments:**
- `<issue>` - GitHub issue number (e.g., `42`) or full URL (e.g., `https://github.com/wanaku-ai/wanaku-capabilities-java-sdk/issues/42`)
...
- If full URL: extract number from path (e.g., `42` from `https://github.com/wanaku-ai/wanaku-capabilities-java-sdk/issues/42`)
...
```bash
gh issue view <ISSUE_NUMBER> --repo wanaku-ai/wanaku-capabilities-java-sdk --json number,title,body,state,labels,assignees,milestone
```

Or via API:

```bash
curl -s "https://api.github.com/repos/wanaku-ai/wanaku-capabilities-java-sdk/issues/<ISSUE_NUMBER>" | jq '{
  number: .number,
  title: .title,
  body: .body,
  state: .state,
