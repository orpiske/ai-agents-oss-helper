...
Create a new issue in the Wanaku GitHub repository.

## Usage

```
/wanaku-create-issue <title>
```
...
```bash
gh issue create --repo wanaku-ai/wanaku --title "<TITLE>" --label "<LABELS>" --body "$(cat <<'EOF'
<BODY_CONTENT>
EOF
)"
```

Or with a HEREDOC for complex bodies:

```bash
gh issue create --repo wanaku-ai/wanaku --title "<TITLE>" --label "<LABELS>" --body "<BODY>"
```
...
Issue #123: https://github.com/wanaku-ai/wanaku/issues/123
```
...
- Issue is created in the Wanaku repository
