...
Apply a quick fix to the Wanaku codebase without requiring a tracked issue. Intended for small changes such as CI fixes, documentation fixes, dependency upgrades, and other minor improvements.

## Usage

```
/wanaku-quick-fix <description>
```

**Arguments:**
- `<description>` - A brief description of the quick fix to apply (e.g., `"upgrade Quarkus to 3.18.0"`, `"fix typo in README"`, `"update CI workflow to use Java 21"`)
...
If the change is too large or complex (e.g., new features, architectural changes, refactoring), stop and suggest creating an issue with `/wanaku-create-issue` instead.
